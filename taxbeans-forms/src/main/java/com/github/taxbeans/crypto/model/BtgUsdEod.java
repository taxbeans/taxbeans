...
public class BtgUsdEod {
	...
	final static Logger logger = LoggerFactory.getLogger(BtgUsdEod.class);

	private static Map<LocalDate, BtgUsdEod> map;
	...
	private static Object addToMap(Map<LocalDate, BtgUsdEod> map, String[] line) {
		BtgUsdEod dataPoint = new BtgUsdEod();
	...
	public static MonetaryAmount getClose(ZonedDateTime zonedDateTime) {
		...
		BtgUsdEod coinMarketCapEthEod = map.get(utcZonedDateTime.toLocalDate());
		if (coinMarketCapEthEod == null) {			
			throw new AssertionError(String.format("No %1$s data available on date: %2$s", BTG, utcZonedDateTime.toLocalDate()));
		}
	...
	public static CommodityExchangeRate getRate(ZonedDateTime zonedDateTime) {
		...
		Commodity digitalCommodity = Commodity.commodity().withSymbol(BTG).build();
