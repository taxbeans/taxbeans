...
public class BtcUsdEod {
	...
	final static Logger logger = LoggerFactory.getLogger(BtcUsdEod.class);

	private static Map<LocalDate, BtcUsdEod> map;
	...
	private static Object addToMap(Map<LocalDate, BtcUsdEod> map, String[] line) {
		BtcUsdEod dataPoint = new BtcUsdEod();
	...
	public static MonetaryAmount getClose(ZonedDateTime zonedDateTime) {
		...
		BtcUsdEod coinMarketCapEod = map.get(utcZonedDateTime.toLocalDate());
		if (coinMarketCapEod == null) {
			throw new AssertionError(String.format("No %1$s data available on date: %2$s", BTC, utcZonedDateTime.toLocalDate()));
		}
	...
	public static CommodityExchangeRate getRate(ZonedDateTime zonedDateTime) {
		...
		Commodity digitalCommodity = Commodity.commodity().withSymbol(BTC).build();
