...
public class XrpUsdEod {
	...
	final static Logger logger = LoggerFactory.getLogger(XrpUsdEod.class);

	private static Map<LocalDate, XrpUsdEod> map;

	public static Map<LocalDate, XrpUsdEod> loadFromCSV() {
		List<String[]> parsedFile = CSVParser.newInstance().parseFile(
				"target/classes/trades/XRP-USD.csv", true);
		Map<LocalDate, XrpUsdEod> map = new HashMap<>();
		parsedFile.forEach(line -> addToMap(map , line));
		XrpUsdEod.map = map;
		return map;
	}

	private static Object addToMap(Map<LocalDate, XrpUsdEod> map, String[] line) {
		XrpUsdEod dataPoint = new XrpUsdEod();
	...
	public static MonetaryAmount getClose(ZonedDateTime zonedDateTime) {
		...
		XrpUsdEod coinMarketCapEthEod = map.get(utcZonedDateTime.toLocalDate());
		if (coinMarketCapEthEod == null) {			
			throw new AssertionError(String.format("No %1$s data available on date: %2$s", XRP, utcZonedDateTime.toLocalDate()));
		}
	...
	public static CommodityExchangeRate getRate(ZonedDateTime zonedDateTime) {
		...
		Commodity digitalCommodity = Commodity.commodity().withSymbol(XRP).build();
